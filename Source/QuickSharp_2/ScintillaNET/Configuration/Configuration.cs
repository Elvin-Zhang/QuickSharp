	...
	public class Configuration
	{
				...
				while (reader.MoveToNextAttribute())
				{
					...
					switch (attrName)
					{
						case "flags":
							...
							if (flags != string.Empty)
							{
                                // QuickSharp change (can't |= a null)
                                FoldFlag? ff = null;
                                foreach (string flag in flags.Split(' '))
                                {
                                    FoldFlag f = (FoldFlag)Enum.Parse(typeof(FoldFlag), flag.Trim(), true);
                                    if (ff == null)
                                        ff = f;
                                    else
                                        ff |= f;
							...
							break;
                        // QuickSharp change - IsEnabled should be lowercase
						case "isenabled":
		...
		private void readCommands(XmlReader reader)
		{
			if (reader.HasAttributes)
			{
				while (reader.MoveToNextAttribute())
				{
                    // QuickSharp changes - attribute names should be lowercase
					string attrName = reader.Name.ToLower();
					switch (attrName)
					{
						case "inherit":
