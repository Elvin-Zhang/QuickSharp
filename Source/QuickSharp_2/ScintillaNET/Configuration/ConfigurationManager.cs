...
namespace ScintillaNet.Configuration
{
	...
	public class ConfigurationManager : ScintillaHelperBase
	{
		...
		private string userFolder
		{
			get
			{
				...
				if (_userFolder == null)
				{
                    ...
                    // QuickSharp changes here
                    QuickSharp.Core.ApplicationManager applicationManager = 
                        QuickSharp.Core.ApplicationManager.GetInstance();

                    _userFolder = Path.Combine
                        (applicationManager.QuickSharpUserHome, "Scintilla");

                    /*
                     * Use theme specific sub-folder if it exists. We don't
                     * create it as it may be missing because we want to use
                     * the default.
                     */

                    string theme = applicationManager.GetSelectedThemeProviderKey();

                    if (!String.IsNullOrEmpty(theme))
                    {
                        string themeFolder = Path.Combine(_userFolder, theme);

                        if (Directory.Exists(themeFolder))
                            _userFolder = themeFolder;
                    }
		...
		internal void Configure(List<Configuration> configList)
		{
            ...
            // QuickSharp change - FoldMarkerScheme moved back again!
            // Setting the folding scheme seems to reset the marker colors.
            ...
            // QuickSharp changes - Color and Highlight color missing for fold margins
            Color? fmc = null;
            foreach (Configuration c in configList)
            {
                if (!c.Margin_List.FoldMarginColor.IsEmpty)
                    fmc = c.Margin_List.FoldMarginColor;
            }
            if (fmc.HasValue)
                Scintilla.Margins.FoldMarginColor = (Color)fmc;

            Color? fmhc = null;
            foreach (Configuration c in configList)
            {
                if (!c.Margin_List.FoldMarginHighlightColor.IsEmpty)
                    fmhc = c.Margin_List.FoldMarginHighlightColor;
            }
            if (fmhc.HasValue)
                Scintilla.Margins.FoldMarginHighlightColor = (Color)fmhc;
